...
def register_benchmarks(overwrite: bool = False) -> None:
    flexs = [
        Benchmark(
            ...
            workdir_command="cat metadata.json | cd $(jq -r .buggyPath) && pwd",
        )
        for i in range(1, 7)
    ]
    ...
    spearmints = [
        Benchmark(
            ...
            workdir_command="export DEBIAN_FRONTEND=non-interactive && apt-get update >/dev/null 2>&1 && apt-get install -y --no-install-recommends jq >/dev/null 2>&1 && cat metadata.json | cd $(jq -r .buggyPath) && pwd",
        )
        for i in range(1, 2)
    ]
